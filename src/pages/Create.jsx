    ...
    const handleCardChange = (e, index) => {
        ...
        if(name === 'prompt'){
            ...
            timer = setTimeout(async() => {
                ...
                const translate = res.matches.sort((a, b) => b["usage-count"] - a["usage-count"])
                termArr[index] = { ...termArr[index], answer: translate[0].translation }
                setTerms(termArr)
            }, 600)
        }else{
            if(value === "") return
  ...
  return (
    ...
    <div className="create">
        ...
        <div className="create__top" ref={topRef}>
            <div className="create__top__left">
                Tạo học phần mới
            </div>
        ...
        <div className="create__header">
            <div className="create__header__left">
                <div className="create__header__input">
                    <div className="create__header__input__wrap">
                        <textarea maxLength="30" placeholder='Nhập tiêu đề...' onChange={handleChange} value={values.title} name='title' onBlur={handleBlur}
                        >
                        </textarea>
                        <small>{values.title.length}/30</small>
                    </div>
                    <p>tiêu đề {errors.title && touched.title && (
                    <span className="errorMsg"> {errors.title}</span>
                    )}</p>
                </div>
                <div className="create__header__input">
                    <div className="create__header__input__wrap">
                        <textarea maxLength="200" placeholder='Thêm mô tả' onChange={(e) => setDescription(e.target.value)} value={description}
                        className="create__header__input__wrap__des" rows={4}
                        >
                        </textarea>
            ...
            <div className="create__header__right">
                <div className="create__header__right__item">
                    <div className="create__header__right__item__txt">
                        Ảnh nền
                    </div>
                    <div className="create__header__right__item__color">
                        {
                            colors.map((item, index) => {
                                ...
                                return (
                                    <div className={`create__header__right__item__color__item ${item.css === background ? 'active' : ''}`} key={index} style={style} onClick={() => setBackground(item.css)}></div>
                                )
                            })
                        }
                    </div>
                ...
                <div className="create__header__right__item">
                    <div className='checkbox__private'>
                        <div className="checkbox__private__txt">Share <input type="checkbox" onChange={e => setShare(e.target.checked)} checked={share} value={share}></input></div>
                    </div>
                </div>
            </div>
        </div>
                ...
                terms && terms?.map((item, index) => (
                    <div className="term" key={index}>
                        <div className="term__header">
                            <div className="term__header__count">
                                {index + 1}
                            </div>
                            <div className="term__header__control" onClick={() => handleCardDelete(index)}>
                                <i className="fa-solid fa-trash-can"></i>
                            </div>
                        </div>
                        <div className="term__content">
                            <div className="term__content__input">
                                <textarea name="prompt" maxLength="255" onChange={(e) => handleCardChange(e, index)} value={terms[index].promp}></textarea>               
                                <p>thuật ngữ</p>
                            </div>
                            <div className="term__content__input">
                                <textarea name="answer" maxLength="255" onChange={(e) => handleCardChange(e, index)} value={terms[index].answer}></textarea>
                                <p>định nghĩa</p>
                            </div>
                        </div>
                    </div>
