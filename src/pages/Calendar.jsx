...
import { GetBoard, CreateBoard, UpdateBoard } from '../redux/calendarApi/boardApi';
import { CreateSection, UpdatePositionSection, DeleteSection } from '../redux/calendarApi/sectionApi';
...
const Calendar = () => {
    ...
    const getBoard = async () => {
      ...
      if(res){
        ...
        if(res.board.cardOrder.length > 0){
          ...
          setSections(sortOrder(calendarData.sections, calendarData.board.cardOrder, '_id'))
        }
      }else{
        await CreateBoard(boardTitle, accessToken, dispatch)
    ...
    if(result.removedIndex || result.addedIndex){
      if(result.removedIndex !== result.addedIndex){
        console.log(result);
          ...
          clearTimeout(timer)
          timer = setTimeout(() => {
            UpdatePositionTask(currentSection.taskOrder, currentSection._id, sectionIdAdded, result.payload._id, accessToken)
          }, timeOut)
          setSections(newSections)
      }
    }
  ...
  return (
    ...
    <div className="calendar">
      <Search/>
      {
        loading ? <Skeleton type='calendar'/> : <>
        {
          ...
          >
            {
              ...
              <Draggable key={item._id}>
                <Section data={item} boardId={board?._id} onTaskDrop={onTaskDrop} handleDeleteSection={handleDeleteSection} index={index}/>
              </Draggable>
              ))
            }
              ...
              <div className="calendar__column__top">
                {
                  ...
                  <div className="calendar__column__top__form">
                    <textarea maxLength="50" placeholder='United' value={sectionTitle} onChange={e => setSectionTitle(e.target.value)}/>
                    <div className="calendar__column__top__form__btn">
                      <div className="calendar__column__top__form__btn__add" onClick={() => handleCreateSection(sectionTitle, accessToken)}>
                        Add
                      </div>
                      <div className="calendar__column__top__form__btn__cancel" onClick={() => setOpenFormAdd(false)}>
                        Cancel
                      </div>
                    </div>
                  </div> : 
                  <div className="calendar__column__top__create" onClick={() => setOpenFormAdd(true)}>
