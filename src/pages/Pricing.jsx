...
import { getPries } from "../redux/apiRequest";
...
const Pricing = () => {
  ...
  const onHandleCreateInvoice = async (id) => {
    if (!accessToken) {
      navigate("/login");
      return;
    }
      ...
      <div className="quizlet-banner">
        <div className="banner-content">
          ...
          <p className="description">
            Ưu đãi có giới hạn: Nâng cấp lên gói FluxQuiz Plus với giá chỉ{" "}
            {monthPrice?.price.toLocaleString()}đ cho tháng đầu tiên. Sau đó sẽ
            quay lại giá gốc
          </p>
        </div>
              ...
              return (
                <div className="subscription-card" key={index}>
                  ...
                  <div>
                    ...
                    <p className="price">
                      {item.price.toLocaleString()}đ<span> /tháng</span>
                    </p>
                  </div>
                  <button
                    className="start-trial-btn"
                    onClick={() => onHandleCreateInvoice(item._id)}
                  >
              ...
              return (
                <div className="subscription-card" key={index}>
                  ...
                  <div>
                    <p className="card-description">{item.description}</p>
                    <p className="price">
                      {Math.floor(item.price / 12).toLocaleString("vi-VN")}đ
                      <span> /tháng</span>
                    </p>
                  </div>
                  <button
...
const FAQ = () => {
  ...
  const faqData = [
    ...
    {
      question:
        "Gói thành viên của tôi có áp dụng cho toàn bộ nguồn gốc không?",
      answer:
    ...
    {
      question:
        "Tôi có thể học nhiều hơn một nguồn gốc trên tài khoản của mình không?",
      answer:
    ...
    },
    {
      ...
      answer:
        "Nếu bạn chọn gói Trọn đời, số tiền sẽ được tính dưới dạng thanh toán trả trước một lần mà không cần gia hạn và bạn sẽ sở hữu trạng thái Pro mãi mãi.",
    },
    {
      question: "Tôi có thể sử dụng tài khoản trên nhiều thiết bị được không?",
      answer:
        "Được! Bạn có thể truy cập hồ sơ Memrise và sử dụng gói thành viên Pro của mình trên nhiều thiết bị: chỉ cần đăng nhập với các thông tin chi tiết giống nhau trên ứng dụng di động iOS và Android hoặc trên máy tính của bạn.",
    },
  ];
