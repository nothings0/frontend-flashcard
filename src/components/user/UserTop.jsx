    ...
    <div className="user__top">
      <div className="user__top__background">
        {data?.bio ? (
          ...
          <h3>Học tập không giới hạn tại Fluxquiz.com</h3>
        )}
      </div>
      <div className="user__top__info">
        ...
        <div className="user__top__info__name">
          <h2>{data?.name.length > 0 ? data?.name : data?.username}</h2>
          {["MONTHLY", "YEARLY"].includes(data?.plan?.type) ? (
            <span>Pro</span>
          ) : null}
        </div>
      </div>
      {avatar && (
        <div className="user__top__btn">
