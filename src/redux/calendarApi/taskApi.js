...
export const UpdateTask = async (taskId, dataList, accessToken) => {
    try {
        const res = await axiosJWT.put(`${genURL(`/v1/task/${taskId}`)}`, dataList, {
            headers: {token: `Bearer ${accessToken}`}
        })
        return res.data
    } catch (error) {
...
export const UpdatePositionTask = async (taskOrder, sectionId, sectionIdAdded, taskId, accessToken) => {
    try {
        const res = await axiosJWT.put(`${genURL('/v1/task/')}`,{taskOrder, sectionId, sectionIdAdded, taskId}, {
            headers: {token: `Bearer ${accessToken}`}
        })
        return res.data
    } catch (error) {
